...
export default function parseMarkup(text: string) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (regexps.color.test(sliced)) {
      ...
      if (!match || !color) throw new Error("Unknown markup parse error. (001)");
      addTextChunk();
      ...
      if (!match) throw new Error("Unknown markup parse error. (002)");
      addTextChunk();
      elStack.pop();
      i += match.length - 1;
      continue;
