...
const itemList = new ItemList({
  wrapper: document.getElementById("content") as HTMLDivElement,
  type: "perk",
  displayTemplate(_, item) {
    itemImageSprite.update(...item.iconPos);
    const itemName = document.getElementById("item__name") as HTMLImageElement;
    itemName.replaceChildren(parseMarkup(item.getListName()));
    const itemDescription = document.getElementById("item__description") as HTMLDivElement;
    itemDescription.replaceChildren(parseMarkup(item.description));
    const itemExtraMessage = document.getElementById("item__extra-message") as HTMLDivElement;
    itemExtraMessage.replaceChildren(parseMarkup(
      item.extraMessage !== "" ?
        item.extraMessage :
        "[color:#fff2]No extra message.\nAdd any message about item here.[/color]"
    ));
    const itemFrom = document.getElementById("item__from") as HTMLDivElement;
    console.log(item.fromRaidNr, item.fromRaidNr.length >= 1);
    itemFrom.replaceChildren(parseMarkup(
      item.fromRaidNr.length >= 1 ?
        `Section ${item.fromRaidNr.join(", ")}` :
        "[color:#fff2]Nowhere...[/color]"
    ));
  }
});
const itemImageEl = document.getElementById("item__image") as HTMLImageElement;
